...
@section('content')
    <div class="mx-auto max-w-[1500px]">
        ...
        <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="Espacios financieros">
            @if ($espacios->count() > 0)
                @foreach ($espacios as $space)
                    ...
                    <article class="relative rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                        <button type="button"
                            class="absolute right-3 top-3 rounded-md p-1 text-slate-500 hover:bg-slate-100"
                            aria-label="Opciones">
                        ...
                        <div
                            class="mx-auto mt-2 w-fit rounded-full px-3 py-1 text-[10px] font-semibold {{ $colors['pill'] }}">
                            {{ $style['label'] }}</div>
            ...
            @else
                <div
                    class="col-span-full overflow-hidden rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-12 text-center shadow-sm sm:px-10 sm:py-16">
                    <div
                        class="mx-auto grid size-20 place-items-center rounded-2xl bg-gradient-to-br from-blue-50 via-cyan-50 to-emerald-50 text-cyan-600 ring-1 ring-cyan-100">
                        <svg class="size-10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"
                            ...
                            <path d="M7 6V4h10v2" />
                            <circle cx="16" cy="13" r=".75" fill="currentColor" stroke="none" />
                        </svg>
                    </div>
                    <h2 class="mt-6 text-xl font-bold tracking-tight text-slate-950">Crea tu primer espacio financiero
                    </h2>
                    <p class="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">
                        Organiza tus ingresos, gastos, presupuestos y metas en un solo lugar. Empieza creando un espacio
                        para ti, tu familia o tu negocio.
                    </p>
                    <button command="show-modal" commandfor="crear-espacio-financiero" type="button"
                        class="mt-7 inline-flex h-11 items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-[#087fe5] via-[#16bfc0] to-[#70d95c] px-5 text-sm font-semibold text-white shadow-lg shadow-cyan-200/60 transition hover:-translate-y-0.5 hover:brightness-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 focus-visible:ring-offset-2">
                        <svg class="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
        ...
        @foreach ($espacios as $space)
            <x-espacio-financiero-form :tipos-espacios="$tiposEspacios" :espacio="$space"
